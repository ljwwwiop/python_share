...
def download_song(music_dict):
    ...
    for song_id in music_dict:
        ...
        # #下载地址(地址填写自己的地址)
        # path="D:\\网易云音乐\\%s.mp3"%music_dict[song_id]#通过键值对来查找歌曲名字
        # #下载音乐  urlretriver (地址  路径)
        # time.sleep(1)
        # urlretrieve(song_url,path)
        # print("正在下载%s"%music_dict[song_id])

        response_mp3 = requests.get(song_url,stream=True)
        name = music_dict[song_id]
        with open('D:\\网易云音乐\\%s.mp3'%name,'ab') as f:
